  ...
  return (
    <div className='bg-slate-200 w-full min-h-screen'>
      ...
      {Categories.map((item,id)=>{
        return <div key={id} className='w-[140px] h-[150px] bg-white flex flex-col items-start gap-5 p-5 
    ...
    <div className={`w-full md:w-[40vw] h-[100%] fixed top-0 right-0 bg-white shadow-xl p-6
      transition-all duration-500 flex flex-col items-center overflow-auto ${showCart?"translate-x-0":"translate-x-full"} `}>
        <header className='w-[100%] flex justify-between items-center'> 
          ...
          <RxCross2  className='w-[30px] h-[20px] text-green-400 text-[18px] font-semibold cursor-pointer hover:text-gray-600' onClick={()=>setShowCart(false)}/>
        </header>
        ...
        <div className='w-full flex justify-between items-center p-9'>
          ...
          <button className='w-[80%] p-3 rounded-lg bg-green-500 text-white 
        hover:bg-green-400 transition-all'>place Order</button>
        </>:
